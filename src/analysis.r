...
# Note: not all categorical variables are needed
df$CUSIPShort <- factor(df$CUSIPShort)
df$Quarter <- factor(df$Quarter)
df$SIC <- factor(df$SIC)

# Note: This is commented, because we want to keep it as a percentage
...
# Reorganize columns and eliminate unnecessary columns by only including the
# following rows in the given order
df <- df[c("X", "SIC3", "Name", "Year",
           "MarketValueC", "MarketValue", "MarketValue_lag1", "MarketValue_lag2",
           "RDIntensityC", "RDIntensity", "RDIntensity_lag1", "RDIntensity_lag2",
           "ROAC", "ROA", "ROA_lag1", "ROA_lag2",
